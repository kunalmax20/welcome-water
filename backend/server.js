...
app.post('/api/products', (req, res) => {
  try {
    ...
    if (!name || !category || !unit || !supplier) {
      return res.status(400).json({ success: false, error: 'Missing required fields: name, category, unit, supplier.' });
    }
    const db = readDB();
    const newProduct = {
...
app.post('/api/products/incoming', (req, res) => {
  try {
    ...
    if (!productId || !qty || !supplier) {
      return res.status(400).json({ success: false, error: 'Missing required fields: productId, qty, supplier.' });
    }
    ...
    writeDB(db);
    res.json({ success: true, message: `Stock updated. New balance: ${product.stock} ${product.unit}.`, data: { product, log: logEntry } });
  } catch (err) {
    res.status(500).json({ success: false, error: 'Failed to process incoming stock.' });
  }
});
...
app.post('/api/products/outgoing', (req, res) => {
  try {
    ...
    if (!productId || !qty || !destination) {
      return res.status(400).json({ success: false, error: 'Missing required fields: productId, qty, destination.' });
    }
    const quantity = parseInt(qty);
    if (isNaN(quantity) || quantity <= 0) {
      return res.status(400).json({ success: false, error: 'Quantity must be a positive integer.' });
    ...
    db.activityLogs.unshift(logEntry);
    writeDB(db);
    res.json({ success: true, message: `Dispatched ${quantity} ${product.unit}. Remaining: ${product.stock}.`, data: { product, log: logEntry } });
  } catch (err) {
    res.status(500).json({ success: false, error: 'Failed to process outgoing dispatch.' });
  }
});
...
app.get('/api/health', (req, res) => {
  res.json({ success: true, message: 'Welcome Water backend is running.', timestamp: new Date().toISOString() });
});
